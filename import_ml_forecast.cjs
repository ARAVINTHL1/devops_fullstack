...
db.on('connected', async () => {
  try {
    ...
    if (!fs.existsSync(forecastFile)) {
      console.error('❌ Forecast file not found:', forecastFile);
    ...
    console.log(JSON.stringify(forecastData, null, 2).substring(0, 500), '...');
    ...
    if (forecastData.forecast && forecastData.forecast.dates) {
      forecastData.forecast.dates.forEach((date, index) => {
        forecasts.push({
          ...
          actual: null, // Actual data not available yet for future dates
          predicted: Math.round(forecastData.forecast.values[index]) || 126
        });
      });
    }

    if (forecasts.length === 0) {
      // Fallback: Create mock forecast data for 7 days
      ...
      for (let i = 0; i < 7; i++) {
        ...
        forecasts.push({
          ...
          actual: Math.floor(Math.random() * 50) + 100, // Random actual between 100-150
